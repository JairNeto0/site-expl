...
export async function queryReport(sql, params = []) {
	const result = await pool.query(sql, params);

	return result.rows;
}

function createReportError(statusCode, message) {
...
function toOptionalPositiveInteger(value) {
	...
	if (!Number.isInteger(parsedValue) || parsedValue <= 0) {
		throw createReportError(400, `Parâmetro numérico inválido: ${value}`);
	}
...
function toOptionalBoolean(value) {
	...
	throw createReportError(400, `Parâmetro booleano inválido: ${value}`);
}
...
function appendCondition(clauses, params, sqlSnippet, value) {
	...
	params.push(value);
	clauses.push(`${sqlSnippet} $${params.length}`);
}
...
export async function getFaturamentoTotal(filters = {}) {
	...
	const [report] = await queryReport(
		`
			SELECT
				COALESCE(SUM(valor_recebido), 0)::float8 AS faturamento_total,
				COALESCE(COUNT(*), 0)::int AS total_transacoes
	...
	const data = await queryReport(
		`
			SELECT
				...
				p.estoque_atual,
				p.ativo,
				COUNT(ip.id_item)::int AS total_itens_vendidos,
				COUNT(DISTINCT ip.id_pedido)::int AS total_pedidos
			FROM itens_pedido ip
			JOIN produtos p ON p.id_produto = ip.id_produto
			${buildWhereClause(clauses)}
			GROUP BY ip.id_produto, p.id_categoria, p.preco_venda, p.custo_unitario, p.estoque_atual, p.ativo
			ORDER BY total_itens_vendidos DESC, ip.id_produto ASC
...
export async function getClientesComMaisCompras(filters = {}) {
	...
	if (normalizedFilters.data_inicio) {
		...
		clauses.push(`u.data_criacao::date >= $${params.length}::date`);
	}
	...
	const data = await queryReport(
		`
			SELECT
				...
				COALESCE(SUM(p.valor_total), 0)::float8 AS valor_total_comprado
			FROM usuarios u
	...
	return buildReportPayload('Clientes com mais compras', 'clientes-com-mais-compras', data, normalizedFilters);
}
	...
	const data = await queryReport(
		`
			SELECT
				...
				p.ativo
			FROM produtos p
			WHERE p.estoque_atual <= $${params.length + 1}
			${clauses.length ? `AND ${clauses.join(' AND ')}` : ''}
			ORDER BY p.estoque_atual ASC, p.id_produto ASC
			LIMIT $${params.length + 2}
		`,
		[...params, normalizedFilters.threshold, normalizedFilters.limit]
	);
...
export async function getReportsDashboard() {
	const [faturamentoTotal, ticketMedio, produtosMaisVendidos, clientesComMaisCompras, estoqueBaixo] = await Promise.all([
		getFaturamentoTotal(),
		getTicketMedio(),
...
export async function getReportData(reportKey, filters = {}) {
	switch (reportKey) {
		...
		default:
			throw createReportError(404, `Relatório "${reportKey}" não encontrado`);
	}
}
